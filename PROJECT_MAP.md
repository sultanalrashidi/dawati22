...
- The scene flow's `scroll-snap-type: y mandatory` + `scroll-snap-stop: always` combination reads fine on desktop trackpad/wheel testing but the user reported it as effectively one-way on a real phone — mandatory snap + always-stop fights momentum/reverse swipes on real touch devices (a known CSS scroll-snap gotcha, not reproducible via synthetic wheel events). Also found: `100vh` doesn't match the visible viewport on mobile browsers whose chrome (address bar) resizes dynamically, and the admin/theme-gallery preview dialog had its own `overflow-y-auto` on `Dialog.Popup` nested around `InvitationView`'s own scroll container, risking scroll-chaining. Fixed all three (`y proximity` + no forced stop + `overscroll-behavior-y: contain`, `100dvh` with a `100vh` fallback, `Dialog.Popup` now `overflow-hidden`) **and**, since gesture physics still vary across browsers, added real forward/back chevron buttons to every scene (not just decorative hints) that `scrollIntoView` the next/previous DOM sibling — a fallback that works regardless of swipe quirks. Verified programmatically: forward-forward-back-back from scene 1 returns exactly to scrollTop 0.
