...
**Post-open scene flow (universal, all themes)**: after the cover opens, every theme now shows the same sequence of full-height, scroll-snapped scenes instead of one static card — reveal (names + Arabic invitation text + optional countdown), personalization (guest name inside a thin corner-bracket frame), details (date/time/location + map), then RSVP or the accepted/declined outcome (QR pass or apology message). Each scene fades/slides in the first time it's ~30% visible (`useInView` + `IntersectionObserver` in `invitation-view.tsx`, CSS in `globals.css` under `.dawati-scene*`). Ported from a reference mockup's own scene structure, applied to all 47 themes without exception per explicit request — only the cover screen (which still uses each theme's own open-style: plain scan-line indicator, wax seal, or doors), palette, fonts, decoration kit, and (for `split-portrait` layouts) a persistent side accent bar still differ per theme. The 4 card "layouts" (`classic-center`/`arch-frame`/`envelope-reveal`/`split-portrait`) now only shape the **cover** screen, not the post-open content — a deliberate simplification versus re-deriving 4 distinct scene templates.

**Decoration kit redesigns**: `LuxuryKit`'s original curved corner flourish and `ExperimentalKit`'s floating dots were both explicitly disliked by the user ("looks old" / "must change"). Replaced with: `LuxuryKit` → 4 small quarter-round corner brackets (same visual language as the new personalization-scene frame); `ExperimentalKit` → two faint vertical guide lines plus a single accent line that continuously scans top-to-bottom (`dawati-decor-scan` in `globals.css`), a more contemporary/techy motif.
