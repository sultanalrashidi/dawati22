...
- Theme engine: 3 real starter themes (`palace-gold`/arch-frame/dark-luxury, `modern-minimal`/classic-center, `botanical-garden`/envelope-reveal) — genuinely distinct layout, fonts, palette, and motion, not just recolors.
...
**Explicitly deferred** (see `web/README.md` for the full list): live Moyasar/Authentica traffic (real code, needs real env keys), Mada/Apple Pay (shown as "coming soon", same Moyasar integration point), expanding the theme library beyond 3 exemplars (now a content task via the admin theme manager, not an engineering one), an automated test suite (verification here was a manual tool-driven pass through every surface + a Postgres-level check after each), and a couple of smaller admin conveniences not in the original spec text (e.g. customer-facing guest-management-request review queue, in-app notification delivery beyond the DB record).
