...
**Optional background music**: `Event.musicYoutubeId` (nullable) is set by the customer as a plain YouTube link at event creation, parsed via `lib/youtube.ts`. The guest page never autoplays — a vinyl-icon toggle (`MusicToggle` in `invitation-view.tsx`) only appears when a link was set, and only starts a hidden `youtube-nocookie.com` iframe (controlled via `postMessage`) on explicit click.
